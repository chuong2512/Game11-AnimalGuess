...
public class QSlot : MonoBehaviour
{
    ...
    public void OnClick()
    {
        ...
        {
            isPresent = false;
            MainGameManager.Instance.FillQSlot(slotText.text.ToUpper(), transform.GetSiblingIndex());
    ...
    {
        MainGameManager.Instance.qSlotLst[returningIndex].Reset(text);
    }
    public void Reset(string va)
    {
        isPresent = true;
        slotText.text = va.ToUpper();
