...
public class FakeUIManager : MonoBehaviour
{
    ...
    public void FbUrl()
    {
        if (count == 0)
        {
            MainGameManager.Instance.CurrentCoins -= 5;
    ...
    private static FakeUIManager _instance;

    public static FakeUIManager Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = GameObject.FindObjectOfType<FakeUIManager>();
            }
            return _instance;
        }
    }
